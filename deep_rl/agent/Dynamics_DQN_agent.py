...
class Dynamics_DQN_Agent(BaseAgent):
    ...
    def train_mdp(self, alpha=None, train_steps=None):
        ...
        for trian_iter in range(train_steps):
            ...
            next_states = next_states.unsqueeze(1).repeat(1, config.particles, 1).detach()
            ...
            autograd.backward(pred_rewards_i, grad_tensors=svgd_reward.detach(), retain_graph=True)

            ## dynamic function svgd 
            dynamic_loss = F.mse_loss(
                pred_states_j, target_states_j.detach(), reduction='none')
            dynamic_grad = autograd.grad(dynamic_loss.sum(), inputs=pred_states_j)[0]
            dynamic_grad = dynamic_grad.unsqueeze(2) # [batch, particles//2, 1, d_state]

            # grad_kappa: [batch, particles/2, particles/2, d_state+1]
            kappa_dynamic, grad_kappa_dynamic = batch_rbf_xy(pred_states_j, pred_states_i)
            kappa_dynamic = kappa_dynamic.unsqueeze(-1)  # [batch, particles/2, particles/2, 1]

            # [batch, particles/2, particles/2, d_state+1]
            kernel_dynamic_grad = torch.matmul(kappa_dynamic.detach(), dynamic_grad)
            # [batch, particles/2, d_state+1]
            svgd_dynamic = (kernel_dynamic_grad + alpha * grad_kappa_dynamic).mean(1)

            autograd.backward(pred_states_i, grad_tensors=svgd_dynamic.detach())
            ...
            self.logger.add_scalar(
                'dynamic_loss', dynamic_loss.mean(), self.total_steps)
            self.logger.add_scalar('dynamic_kappa', kappa_dynamic.mean(), self.total_steps)
            self.logger.add_scalar(
                'dynamic_grad_kappa', grad_kappa_dynamic.mean(), self.total_steps)
