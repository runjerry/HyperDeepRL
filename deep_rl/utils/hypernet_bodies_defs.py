...
def FCBody_config(d_state, d_hidden, gate, mixer=False):
    net_config = {}
    net_config['fc1'] = fc_body(d_input=d_state, d_output=d_hidden[0])
    for i in range(1, len(d_hidden)):
        net_config['fc{}'.format(i+1)] = fc_body(d_input=d_hidden[i-1], d_output=d_hidden[i])
    if mixer:
        net_config['mixer'] = fc_body(d_hidden=64, d_output=None, n_gen=len(d_hidden))
    net_config['z_dim'] = 64
    net_config['s_dim'] = 64
    net_config['n_gen'] = len(d_hidden)
