...
def ToyFCBody_config(d_state, d_hidden, gate, mixer=False):
    net_config = {}
    net_config['fc1'] = toy_fc_body(d_input=d_state, d_output=d_hidden[0], z_dim=2)#4)
    for i in range(1, len(d_hidden)):
        net_config['fc{}'.format(i+1)] = toy_fc_body(d_input=d_hidden[i-1], d_output=d_hidden[i], z_dim=2)#4)

    net_config['z_dim'] = 2#4
    net_config['s_dim'] = 2#4
...
def FCBody_config(d_state, d_hidden, gate, mixer=False):
    net_config = {}
    net_config['fc1'] = fc_body(d_input=d_state, d_output=d_hidden[0], z_dim=11)
    for i in range(1, len(d_hidden)):
        net_config['fc{}'.format(i+1)] = fc_body(d_input=d_hidden[i-1], d_output=d_hidden[i], z_dim=11)
    if mixer:
