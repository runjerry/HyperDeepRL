...
def ToyDuelingNet_config(feature_dim, action_dim, mixer=False):
    net_config = {
            'fc_value': toy_dueling_net(d_input=feature_dim, d_output=1, z_dim=16),
            'fc_advantage': toy_dueling_net(d_input=feature_dim, d_output=action_dim, z_dim=16),
            'z_dim': 16,
            's_dim': 16,
            'n_gen': 2
            }
...
def DuelingNet_config(feature_dim, action_dim, mixer=False):
    net_config = {
            'fc_value': dueling_net(d_input=feature_dim, d_output=1, z_dim=16),
            'fc_advantage': dueling_net(d_input=feature_dim, d_output=action_dim, z_dim=16),
            'z_dim': 16,
            's_dim': 16,
            'n_gen': 2
            }
